  ...
  for(i = 0; i < n; i++)
      ...
      if(mask[i])
	{
	  ...
	  while(fin  - deb > 1)
		...
		fin = val;
	      //cout << "d = " << deb << " f = " << fin << "  k = " << (int)val << "  im = " << (int)img[i] << "val = " << 255.0 * d_ref[val] << endl;
	    }
	  img[i] = val;
	}
    }
...
{
  char nomImg[250], nomMask[250], suf[250], fimg[250], nomref[250];;
  int nH, nW, nTaille, ind, nbimg;
  
  OCTET *ImgIn, *mask, *ref;
  if (argc != 7) 
    {
      printf("Usage: nomImg indice suffixe Mask ref nbimg?\n"); 
      exit (1) ;
    }
