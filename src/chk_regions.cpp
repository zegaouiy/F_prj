...
void chk_window(char* name, int ind, char* suf, int nbimg, int* img, OCTET* mask, int size_max, int size_min, int wh, int ww, int h, int w, char* outname, int gt, int day)
{
  ...
  char out_name[250], dir[250] = "";
  float step = 0.2;

  ofstream count, roc;
  sprintf(out_name, "comptage_jour_%d.dat", day);
  count.open(out_name);
  sprintf(out_name, "roc_jour_%d.dat", day);
  roc.open(out_name);
  ...
  allocation_tableau(crit_size, OCTET, h*w);
  ...
  for(j = 0; j < nbimg; j++)
    {
      ...
      sprintf(out_name, "%s%d%s", name, ind + j, suf);
      read_map(out_name, map, n, sizes);
      sprintf(out_name, "red_%s%d%s", name, ind + j, suf);
      read_map(out_name, red_map, n, red_size);

      i = red_size.size();
      allocation_tableau(center, int, 4 * i); // mini maxi minj maxj
  
      cout << "begin filter ||| isize = " << i << "   size = " << sizes.size() << endl;
      red_filter(red_map, center, red_size, red_tab, h, w);
      cout << "filter done" << endl;
      sprintf(out_name, "%sred_size_%d.pgm", dir, ind+j);
      lire_image_pgm(out_name, crit_size, n);
      sprintf(out_name, "%sred_ratio_%d.pgm", dir, ind+j);
      lire_image_pgm(out_name, crit_ratio, n);
      sprintf(out_name, "%strue_dens_%d.pgm", dir, ind+j);
      ...
      //sprintf(out_name, "%s%d.ppm", outname, ind+j-1);
      //lire_image_ppm(out_name, out, h * w);
      
      cout << "begin draw window" << endl;
      //      draw_window(out, regions, ind+j, wh, ww, h, w);
      cout << "end draw window" << endl;

      if(gt)
	{
	  sprintf(out_name, "gt%d.pgm", ind+j);
	  lire_image_ppm(out_name, out, h * w);
	}
	
      chk_dat(crit_dens, regions, count, roc, wh, ww, h, w, j, gt);
